...
// --- real web search via Brave API (no LLM) ---
...
const PRINCIPLES = [
  ...
  "Pattern mirrors elsewhere.",
  "Nothing rests; observe change.",
  "Opposites differ by degree.",
  "Flows in cycles.",
...
function synthesize(query, findings, hadMemory) {
  ...
  const resolvedBy = findings.length > 0
  ...
  for (const f of findings) {
    ...
    for (const w of words) {
      if (w.length > 3) termFreq[w] = (termFreq[w] || 0) + 1;
    }
  }
...
// --- main resolve with KV cumulative memory ---
...
async function resolveQuery(query, env) {
  ...
  // 3. load cumulative memory from KV
  let historicalMemory = [];
  if (env.KV) {
    const raw = await env.KV.get(memKey);
    if (raw) {
      try {
        historicalMemory = JSON.parse(raw) || [];
      } catch {}
    }
  }
  ...
  if (env.KV) {
    await env.KV.put(
      cacheKey,
      JSON.stringify({ response, trail, resolvedBy }),
      { expirationTtl: 3600 }
    );
  }
